...
def _(
    ...
    logreg_model = pipeline.fit(spark_train)
    print("✅ Model Training Complete!")
    return Label_col, logreg_model


@app.cell
def _(Label_col, MulticlassClassificationEvaluator, logreg_model, spark_test):
    predictions = logreg_model.transform(spark_test)
    evaluator = MulticlassClassificationEvaluator(labelCol=Label_col,
    predictionCol="prediction", metricName="accuracy")
    logisitc_regression_accuracy = evaluator.evaluate(predictions)
    return (logisitc_regression_accuracy,)


@app.cell
def _(logisitc_regression_accuracy):
    logisitc_regression_accuracy
    return
