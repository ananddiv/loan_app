...
def _(
    ...
    pipeline = Pipeline(stages=[
        ...
        assembler,
        DecisionTreeClassifier(labelCol=Label_col, featuresCol="features", seed=42) # The final model stage
    ])

    print("\n--- Starting Model Training (Fitting the Pipeline) ---")
    # Fit the pipeline to the entire dataset (Training phase)
    DeciosionTreeModel = pipeline.fit(spark_train)
    print("✅ Model Training Complete!")
    return DeciosionTreeModel, Label_col
...
def _(
    ...
    predictions = DeciosionTreeModel.transform(spark_test)
